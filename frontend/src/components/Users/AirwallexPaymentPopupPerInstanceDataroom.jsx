...
const SecurityBadge = styled.div`
  ...
  color: #6b7280;
`;
const AirwallexPaymentPopupPerInstanceDataroom = ({
  show,
  onClose,
  payment,
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (response.status === "SUCCEEDED") {
        await axios.post(
          `${apiURLPayment}CreateuserSubscriptionDataRoomPerinstance`,
          {
            company_id: userLogin.companies[0].id,
            created_by_id: userLogin.id,
            amount: finalAmount,
...
export default AirwallexPaymentPopupPerInstanceDataroom;

...
export default function InvestorDocsharing() {
  ...
  const handleCheckboxChange = (id) => {
    ...
    // Filter only checked records into recordsPopup
    const selectedRecords = updatedData.filter((row) => row.shared === true);
    setrecordsPopup(selectedRecords);

    // Set checkbox master state
    setcheckval(selectedRecords.length > 0);
  };
  ...
  const getInvestorReport = async () => {
    ...
    try {
      const generateRes = await axios.post(
        apiURLINFile + "getInvestorReportlatest",
        formData,
        {
          headers: {
                ...
                <div className="col-md-9">
                  <DataRoomSection className="d-flex flex-column gap-2">
                    <div className="titleroom d-flex justify-content-between align-items-center border-bottom pb-3">
                      <h4>Investor Documents & Sharing</h4>

                              ...
                              db.query(
                                ...
                                (err, pendingInstruments) => {
                                  ...
                                  const preFounderItems = (preFounders || []).map((f) => ({
                                    ...
                                    percentage_formatted: pct(parsePct(f.percentage_formatted)),
                                  ...
                                  const prePoolShares = prePool ? prePool.shares : 0;
                                  const prePoolPct = prePool ? parsePct(prePool.percentage_formatted) : 0;
                                  const prePoolValue = prePool ? (prePoolPct / 100) * preMoneyVal : 0;
                                  ...
                                  const prePrevShares = prePrevInv.reduce((s, i) => s + i.shares, 0);
                                  const prePrevValue = prePrevInv.reduce(
                                    (s, i) => s + (parsePct(i.percentage_formatted) / 100) * preMoneyVal,
                                    0,
                                  );
                                  const preConvShares = preConvInv.reduce((s, i) => s + i.shares, 0);
                                  ...
                                  const preTotalFounderShares = preFounderItems.reduce((s, f) => s + f.shares, 0);
                                  const preTotalFounderValue = preFounderItems.reduce((s, f) => s + f.value, 0);
                                  const preTotalShares =
                                    preTotalFounderShares + prePoolShares + prePrevShares + preConvShares;
                                  const preTotalValue =
                                    preTotalFounderValue + prePoolValue + prePrevValue + preConvValue;

                                  // Group previous investors by round_name
                                  const prePrevGroups = {};
                                  prePrevInv.forEach((i) => {
                                    const key = i.round_name || "Previous Investors";
                                    if (!prePrevGroups[key]) {
                                      prePrevGroups[key] = {
                                        round_name: key,
                                        round_id_ref: i.round_id_ref,
                                        shareClassType: i.share_class_type || i.instrument_type || "",
                                    ...
                                    prePrevGroups[key].total_pct += parsePct(i.percentage_formatted);
                                    prePrevGroups[key].total_value +=
                                      (parsePct(i.percentage_formatted) / 100) * preMoneyVal;
                                  });
                                  ...
                                  const preMoneyCapTable = {
                                        ...
                                        ? [
                                            {
                                              ...
                                              is_option_pool: true,
                                              existing_shares: prePool.existing_shares || prePoolShares,
                                              new_shares: prePool.new_shares || 0,
                                            },
                                          ]
                                        : []),
                                      ...Object.values(prePrevGroups).map((group) => ({
                                        type: "investor",
                                        ...
                                        investor_details: group.items.map((i) => ({
                                          ...
                                          percentage_formatted: pct(parsePct(i.percentage_formatted)),
                                          value: (parsePct(i.percentage_formatted) / 100) * preMoneyVal,
                                          ...
                                          round_id_ref: i.round_id_ref,
                                          investor_details: parseDetails(i.investor_details),
                                          is_previous: true,
                                        })),
                                      })),
                                      ...(preConvInv.length > 0
                                    ...
                                    totals: {
                                      ...
                                      total_investors: prePrevShares + preConvShares,
                                  ...
                                  const postPoolExisting = postPool ? postPool.existing_shares || 0 : 0;
                                  const postPoolNew = postPool ? postPool.new_shares || 0 : 0;
                                  const postPoolTotal = postPool ? postPool.shares : 0;
                                  const postPoolPct = postPool ? parsePct(postPool.percentage_formatted) : 0;
                                  const postPoolValue = postPool ? parseFloat(postPool.value || 0) : 0;
                                  ...
                                  const postTotalFounderShares = postFounderItems.reduce((s, f) => s + f.shares, 0);
                                  const postTotalFounderValue = postFounderItems.reduce((s, f) => s + f.value, 0);
                                  const postPrevShares = postPrevInv.reduce((s, i) => s + i.shares, 0);
                                  const postPrevValue = postPrevInv.reduce(
                                    (s, i) => s + parseFloat(i.value || 0),
                                    0,
                                  );
                                  const postConvShares = postConvInv.reduce((s, i) => s + i.shares, 0);
                                  // ✅ DB se value use karo
                                  const postConvValue = postConvInv.reduce(
                                    (s, i) => s + parseFloat(i.value || 0),
                                    0,
                                  );
                                  const postCurrShares = postCurrInv.reduce((s, i) => s + i.shares, 0);
                                  ...
                                  const postTotalShares =
                                    ...
                                    postConvShares +
                                    postCurrShares;
                                  const postTotalNewShares = postPoolNew + postConvShares + postCurrShares;
                                  ...
                                  postPrevInv.forEach((i) => {
                                    ...
                                    postPrevGroups[key].total_shares += i.shares;
                                    postPrevGroups[key].total_pct += parsePct(i.percentage_formatted);
                                    postPrevGroups[key].total_value += parseFloat(i.value || 0);
                                  });

                                  // Converted investors group by round_name
                                  const postConvGroups = {};
                                  postConvInv.forEach((i) => {
                                    const key = i.round_name || "Converted Notes";
                                    ...
                                    postConvGroups[key].total_shares += i.shares;
                                    postConvGroups[key].total_pct += parsePct(i.percentage_formatted);
                                    // ✅ DB se value use karo
                                    postConvGroups[key].total_value += parseFloat(i.value || 0);
                                  });
                                  ...
                                  postCurrInv.forEach((i) => {
                                    ...
                                    postCurrGroups[key].total_shares += i.shares;
                                    postCurrGroups[key].total_new_shares += i.new_shares || i.shares;
                                    postCurrGroups[key].total_pct += parsePct(i.percentage_formatted);
                                    postCurrGroups[key].total_value += parseFloat(i.value || 0);
                                  });

                                  // ✅ Helper: group → item
                                  const buildGroupItem = (group, investorType) => ({
                                    type: "investor",
                                    ...
                                    existing_shares:
                                      investorType === "current" || investorType === "converted"
                                        ? 0
                                        : group.total_shares,
                                    new_shares:
                                      investorType === "current" || investorType === "converted"
                                    ...
                                    is_new_investment: investorType === "current",
                                    ...
                                    investor_details: group.items.map((i) => ({
                                      ...
                                      value: parseFloat(i.value || 0),
                                      value_formatted: money(i.value),
                                      investment_amount: parseFloat(i.investment_amount || 0),
                                  ...
                                  const postMoneyCapTable = {
                                        ...
                                        ? [
                                            {
                                              ...
                                              percentage_formatted: pct(postPoolPct),
                                              value: postPoolValue,
                                              value_formatted: money(postPoolValue),
                                              is_option_pool: true,
                                              instrument_type: postPool.instrument_type || "Options",
                                            },
                                          ]
                                        : []),
                                    ...
                                    totals: {
                                      ...
                                      total_new_shares_formatted: fmt(postTotalNewShares),
                                      total_founders: postTotalFounderShares,
                                      total_option_pool: postPoolTotal,
                                      total_investors: postPrevShares + postConvShares + postCurrShares,

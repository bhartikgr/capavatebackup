...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      // ✅ Check if token expiry exists and is still valid
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(apiURL + "signatorylogin", SaveFormData, {
        headers: {
  ...
  const handleAccessCompany = async (companyId, companyName) => {
    if (!SignatoryLoginData) return;
    let formData = {
      ip_address: ClientIP,
      detail: SignatoryLoginData,
      companyId: companyId,
    ...
    try {
      const res = await axios.post(
        apiURL + "signatoryAccessLastLogin",
        formData,
        {
          headers: {
            Accept: "application/json",
      ...
      // Extract approvedCIO.id
      const approvedCIOId = res.data.approvedCIO?.id;

      // Update user data
      const updatedUserData = {
        ...SignatoryLoginData,
        id: approvedCIOId || SignatoryLoginData.id, // update id if available
        companies: [
          {
            id: companyId, // or you can use res.data.companyId if API returns it
            name: companyName,
          },
        ],
      };
                      ...
                      <div class="row gap-0 dashboard-top p-0 border-0 bg-transparent">
                        <div className="row gy-3 ">
                          <div className="col-12 mb-3">
                            ...
                            >
                              Please choose the company account you’d like to
                              access
                            </p>
                          </div>
                          {companyData?.map((company, index) => (
                            <button
                              key={index}
                              onClick={() =>
                                handleAccessCompany(company.id, company.name)
                              }
                              type="button"
                              className={`border-0 bg-transparent ${companyData.length === 1 ? "col-12" : "col-md-6"
                                }`}
                            >
                              <div
                              ...
                              >
                                <h5
                                  ...
                                  style={{
                                    backgroundColor:
                                      company.company_color_code || "#d40209",
                                    color: "#fff",
                                    padding: "10px 20px",
                                    borderRadius: "8px",
                                    fontSize: "0.9rem",
